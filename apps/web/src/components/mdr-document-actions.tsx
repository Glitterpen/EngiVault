...
export function MdrDocumentActions({
  ...
  return <div className="flex flex-col items-end gap-1.5">
    <div className="flex flex-col items-stretch justify-end gap-1.5 sm:flex-row sm:items-center">
      <Link href={detailPath} className="inline-flex min-h-11 items-center justify-center gap-1.5 rounded-lg border border-[#d8e0dc] px-3 py-2 text-xs font-bold text-[#0c5b45] transition hover:bg-[#eef4f1]" aria-label={`Edit ${documentNumber}`} title="Edit MDR deliverable"><Pencil size={15}/> Edit</Link>
      <form action={action} onSubmit={event=>{if(!window.confirm(prompt))event.preventDefault()}}>
        ...
        <button disabled={pending} aria-busy={pending} className={`inline-flex min-h-11 w-full items-center justify-center gap-1.5 rounded-lg border px-3 py-2 text-xs font-bold transition disabled:cursor-wait disabled:opacity-50 ${archived?"border-[#bdd7cc] text-[#0c5b45] hover:bg-[#eef4f1]":"border-[#efc6b4] text-[#a5452f] hover:bg-[#fff4ef]"}`} aria-label={`${archived?"Restore":"Delete"} ${documentNumber}`} title={archived?"Restore to active MDR":"Delete from active MDR"}>{archived?<ArchiveRestore size={15}/>:<Trash2 size={15}/>} {pending?"Saving…":archived?"Restore":"Delete"}</button>
      </form>
    </div>
