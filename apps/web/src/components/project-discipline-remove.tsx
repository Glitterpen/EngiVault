...
type Props={organisationId:string;projectId:string;name:string};
export function ProjectDisciplineRemove(props:Props){
  const [open,setOpen]=useState(false);
  return <><button type="button" onClick={()=>setOpen(true)} aria-label={`Remove ${props.name}`} aria-haspopup="dialog" className="inline-flex min-h-9 shrink-0 items-center gap-1 rounded-lg px-2 text-xs font-semibold text-[#a53724] hover:bg-[#fff0eb]"><Trash2 size={13}/>Remove</button>
    {open&&<RemovalDialog {...props} close={()=>setOpen(false)}/>}</>;
}

function RemovalDialog({organisationId,projectId,name,close}:Props&{close:()=>void}){
  ...
  return <dialog ref={dialog} aria-labelledby={titleId} onCancel={event=>{event.preventDefault();if(!pending)close();}} className="m-auto max-h-[90dvh] w-[calc(100%_-_2rem)] max-w-lg overflow-y-auto rounded-2xl border border-[#dce2e9] bg-white p-5 text-[#10243e] shadow-2xl backdrop:bg-[#06172b]/60 sm:p-6">
    ...
    <p className="mt-3 text-sm leading-6 text-[#617083]">This removes the discipline from new project selections. Existing engineer access, MDR deliverables, files, invitations and history stay intact. You can restore it later.</p>
    ...
    {!state?.ok&&impact?<form action={action} className="mt-5 space-y-4">
      ...
      <div className="flex flex-wrap justify-end gap-2"><button type="button" className="ev-button-secondary" onClick={close} disabled={pending}>Keep discipline</button><button className="ev-button bg-[#a53724]" disabled={pending}>{pending?"Removing…":"Confirm removal"}</button></div>
...
function Impact({impact}:{impact:DisciplineRemovalImpact}){
  return <div className="mt-4 space-y-3 text-sm">
    ...
    <p className="text-[#617083]">Linked work retained: {impact.documentCount} active MDR deliverable{impact.documentCount===1?"":"s"}, {impact.invitationCount} pending invitation{impact.invitationCount===1?"":"s"} and {impact.plannedPositions} planned position{impact.plannedPositions===1?"":"s"}.</p>
  </div>;
}
