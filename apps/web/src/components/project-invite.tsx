...
export function ProjectInvite({organisationId,projectId,disciplines,bare=false,allowedRoles=["project_admin","document_controller","engineer"],lockedDiscipline}:ProjectInviteProps){
  ...
  return <form onSubmit={event=>{event.preventDefault();void submit(new FormData(event.currentTarget))}} className={bare?"":"ev-card p-6"}>
    ...
    {role==="engineer"&&(lockedDiscipline
      ...
      :<label className="mt-4 block"><span className="ev-label">Authorised discipline</span><select className="ev-input" name="discipline" required value={discipline} onChange={event=>setDiscipline(event.target.value)}><option value="" disabled>Select engineering discipline</option>{disciplines.map(item=><option key={item.name} value={item.name}>{disciplineLabel(item)}</option>)}</select><span className="mt-1 block text-xs leading-5 text-[#617083]">The engineer may upload only to MDR documents in this discipline.</span>{!disciplines.length&&<span className="mt-2 block text-xs font-semibold text-[#a5452f]">No disciplines are available. Use Project disciplines → Add discipline in Project team & resources first.</span>}</label>)}
