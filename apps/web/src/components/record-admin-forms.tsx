...
function DocumentLifecycle({record}:{record:{id:string;organisation_id:string;project_id:string;document_number:string;lifecycle_status:string}}){
 ...
 const prompt=archived?`Restore ${record.document_number} to the active MDR?`:`Remove ${record.document_number} from the active MDR? Its revisions and audit history will be preserved. Its document number can be reused.`;
 return <form action={action} onSubmit={event=>{if(!window.confirm(prompt))event.preventDefault()}} className="ev-card border-[#efc6b4] p-6"><input type="hidden" name="organisationId" value={record.organisation_id}/><input type="hidden" name="projectId" value={record.project_id}/><input type="hidden" name="documentId" value={record.id}/><input type="hidden" name="archived" value={String(!archived)}/><div className={`flex items-center gap-2 ${archived?"text-[#0c5b45]":"text-[#a5452f]"}`}>{archived?<ArchiveRestore size={18}/>:<Trash2 size={18}/>}<h2 className="font-semibold">{archived?"Restore MDR deliverable":"Remove MDR deliverable"}</h2></div><p className="mt-2 text-sm leading-6 text-[#617083]">{archived?"Return this deliverable to the active MDR. If its number has been reused, edit this removed entry to use a different number before restoring it.":"Remove this deliverable from active register views while preserving every revision and audit record."}</p>{state?.message&&<p className={`mt-3 text-sm ${state.mutationId?"text-[#0c5b45]":"text-[#a5452f]"}`} role="status">{state.message}</p>}<button className={archived?"ev-button mt-4":"ev-button-secondary mt-4 border-[#efc6b4] text-[#a5452f]"} disabled={pending}>{pending?"Updating…":archived?"Restore to active MDR":"Remove from active MDR"}</button></form>
}
