...
export function AuthForm({mode,action,next}:{mode:"login"|"register";action:(s:AuthState,d:FormData)=>Promise<AuthState>;next?:string}) {
  ...
  const alternateBase=register?"/login":"/register";
  ...
  return <div className="w-full max-w-md">
    ...
    <p className="mt-2 text-sm text-[#617083]">{invitation?(register?"Create the account using the exact email address that received the project invitation.":"Sign in using the exact email address that received the project invitation."):(register?"Verify your email before creating an organisation.":"Sign in to your authorised organisations and projects.")}</p>
    ...
    <p className="mt-6 text-center text-sm text-[#617083]">{register?"Already registered? ":"New to EngiCite? "}<Link className="font-bold text-[#e8733f] hover:underline" href={alternateHref}>{register?"Sign in":"Create account"}</Link></p>
  </div>
}
