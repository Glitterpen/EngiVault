...
export function FounderMfaGate({
  currentAal,
  continueTo="/founder",
  ...
  purpose?:"access"|"password-recovery";
}){
  ...
  if(currentAal==="aal2")return <div className="ev-card p-7 text-center"><CheckCircle2 className="mx-auto text-[#0c684e]" size={32}/><h2 className="mt-3 text-xl font-semibold">Founder session verified</h2><p className="mt-2 text-sm text-[#617083]">Your high-assurance session is ready.</p><button onClick={()=>router.replace(continueTo)} className="ev-button mt-5">Continue securely</button></div>;

  return <div className="ev-card overflow-hidden">
    <div className="border-b border-[#e2e8ee] bg-[#f8fafc] p-6 sm:p-8"><div className="flex items-start gap-4"><span className="grid size-12 shrink-0 place-items-center rounded-2xl bg-[#10243e] text-white"><ShieldCheck size={23}/></span><div><p className="text-xs font-extrabold uppercase tracking-[.16em] text-[#e8733f]">Step-up security</p><h2 className="mt-1 text-xl font-semibold">Verify with an authenticator app</h2><p className="mt-2 max-w-xl text-sm leading-6 text-[#617083]">{purpose==="password-recovery"?"Changing the founder password requires the current authenticator code in addition to the secure email recovery link.":"Founder access contains cross-organisation business and account information. A password alone is not sufficient."}</p></div></div></div>
    <div className="p-6 sm:p-8">
      {loading?<p className="flex items-center gap-2 text-sm text-[#617083]"><LoaderCircle className="animate-spin" size={17}/> Checking your security factors…</p>:!factor&&!enrollment?allowEnrollment?<div><p className="text-sm leading-6 text-[#526276]">Use Microsoft Authenticator, Google Authenticator, 1Password or another TOTP app. EngiCite will never ask you to send these codes by email.</p><button onClick={beginEnrollment} disabled={submitting} className="ev-button mt-5"><Smartphone size={17}/>{submitting?"Preparing…":"Set up authenticator"}</button></div>:<div className="rounded-xl border border-[#efc1bd] bg-[#fff3f1] p-4 text-sm leading-6 text-[#8f3933]">No verified authenticator is available for this founder identity. Password recovery cannot continue automatically; use the secured founder-account recovery procedure.</div>:<div className="grid gap-7 lg:grid-cols-[auto_1fr]">
        ...
        <div className="self-center"><div className="flex items-center gap-2 text-[#10243e]"><KeyRound size={18}/><h3 className="font-semibold">{factor?"Enter your verification code":"Scan, then verify"}</h3></div><HelpTip label="Authenticator code guidance">Enter the six digits currently shown in your authenticator app.</HelpTip><label className="mt-5 block max-w-xs"><span className="ev-label">6-digit code</span><input value={code} onChange={event=>setCode(event.target.value.replace(/\D/g,"").slice(0,6))} inputMode="numeric" autoComplete="one-time-code" className="ev-input text-center text-lg tracking-[.3em]" placeholder="000000"/></label><button onClick={verify} disabled={submitting||code.length!==6} className="ev-button mt-4">{submitting?<><LoaderCircle className="animate-spin" size={17}/> Verifying…</>:<><ShieldCheck size={17}/> Verify and continue</>}</button></div>
      </div>}
