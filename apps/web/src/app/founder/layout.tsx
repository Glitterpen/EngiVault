...
export default async function FounderLayout({children}:{children:React.ReactNode}){
  ...
  return <div className="min-h-screen bg-[#f3f6f8]">
    <header className="sticky top-0 z-40 border-b border-[#dfe6ec] bg-white/95 backdrop-blur-xl"><div className="mx-auto flex h-16 max-w-[1900px] items-center justify-between px-4 sm:px-6 lg:px-8"><div className="flex items-center gap-3"><Brand href="/founder" compact/><span className="hidden h-7 w-px bg-[#dfe6ec] sm:block"/><span className="hidden items-center gap-2 text-xs font-extrabold uppercase tracking-[.14em] text-[#10243e] sm:flex"><ShieldCheck size={15} className="text-[#e8733f]"/> Founder control centre</span></div><AccountMenu email={user.email??"Founder account"} initialRoleLabel="Founder"/></div></header>
