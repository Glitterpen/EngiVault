...
export async function POST(request:Request,ctx:{params:Promise<{organisationId:string;projectId:string}>}){
  ...
  let discipline:string|undefined;if(parsed.data.role==="engineer"){const {data:categoryRows,error:categoryError}=await supabase.rpc("get_project_document_categories",{target_organisation:organisationId,target_project:projectId});if(categoryError)return Response.json({error:{code:"DISCIPLINES_UNAVAILABLE",message:"Project disciplines could not be loaded. Try again shortly."}},{status:503});discipline=matchDiscipline((categoryRows??[]) as ProjectCategory[],parsed.data.discipline??"");if(!discipline)return Response.json({error:{code:"INVALID_DISCIPLINE",message:"Select an available project discipline. The Project Manager can add disciplines in Project team & resources."}},{status:422})}
