...
const schema=z.object({revisionCode:z.string().trim().toUpperCase().min(1).max(20),issueStatus:z.string().trim().min(2).max(60),issueDate:z.iso.date().optional(),fileName:z.string().min(1).max(180),mimeType:z.string(),size:z.number().int().min(1).max(MAX_UPLOAD_BYTES),sha256:z.string().regex(/^[a-f0-9]{64}$/)});
