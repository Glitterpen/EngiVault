...
export default async function OrganisationPage({params}:{params:Promise<{organisationId:string}>}){
  ...
  return <div className="mx-auto max-w-[1450px]">
    ...
    <section className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><PortfolioMetric label="Active projects" value={activeProjects.length} icon={<FolderKanban/>}/><PortfolioMetric label="Average completion" value={`${average}%`} icon={<Gauge/>}/><PortfolioMetric label="Projects needing attention" value={atRisk} icon={<HeartPulse/>} warn={atRisk>0}/><PortfolioMetric label="Open project issues" value={issueRows.length} icon={<AlertTriangle/>} warn={issueRows.some(issue=>issue.severity==="critical"||issue.severity==="high")}/></section>
