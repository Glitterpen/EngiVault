...
type Project = {project_id:string;code:string;name:string;role:string;status:string};
...
function MemberProjects({organisationId,organisationName,projects,disciplines}:{organisationId:string;organisationName:string;projects:Project[];disciplines:Array<{project_id:string;discipline:string}>}){
  return <div className="mx-auto max-w-6xl">
    ...
    <div className="mt-6 flex flex-col items-start gap-4 sm:flex-row sm:items-center"><span className="grid size-14 shrink-0 place-items-center overflow-hidden rounded-2xl border border-[#dfe7e3] bg-white p-2 shadow-sm sm:size-16"><OrganisationLogo organisationId={organisationId} name={organisationName} size={64} className="size-full object-contain"/></span><div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#e8733f]">Project team workspace</p><h1 className="mt-2 break-words text-2xl font-semibold tracking-[-.04em] sm:text-3xl">My project assignments</h1><p className="mt-2 text-sm leading-6 text-[#617083] sm:text-base">{organisationName} · Open a project to see only the responsibilities assigned to your role.</p></div></div>
    <section className="mt-8 grid gap-4">{projects.length?projects.map(project=>{
      ...
      return <Link href={projectHomePath(organisationId,project.project_id,project.role)} key={project.project_id} className="ev-card group block p-4 transition hover:-translate-y-0.5 hover:shadow-md sm:p-6"><div className="flex items-start gap-3 sm:gap-4"><span className="grid size-11 shrink-0 place-items-center overflow-hidden rounded-xl border border-[#dfe7e3] bg-white p-1.5 text-[#e8733f] sm:size-12"><ProjectLogo organisationId={organisationId} projectId={project.project_id} name={project.name} size={48} className="size-full object-contain"/></span><div className="min-w-0 flex-1"><div className="flex flex-col items-start gap-3 sm:flex-row sm:justify-between"><div className="min-w-0"><p className="text-xs font-bold text-[#e8733f]">{project.code}</p><h2 className="mt-1 break-words text-base font-semibold sm:text-lg">{project.name}</h2><p className="mt-1 text-sm font-medium text-[#0c5b45]">{roleLabel(project.role)}</p></div><span className="inline-flex shrink-0 items-center gap-2 text-sm font-bold text-[#0c5b45]">Open workspace <ArrowRight size={17} className="transition group-hover:translate-x-1"/></span></div>{engineer&&<div className="mt-4 flex flex-wrap items-center gap-2 border-t border-[#edf1ef] pt-4"><span className="w-full text-[10px] font-bold uppercase tracking-[.1em] text-[#617083] sm:w-auto sm:text-xs">Authorised discipline</span>{projectDisciplines.length?projectDisciplines.map(discipline=><span key={discipline} className="rounded-full bg-[#e8f1ed] px-2.5 py-1 text-xs font-bold text-[#0c5b45]">{discipline}</span>):<span className="text-xs text-[#a5452f]">Awaiting discipline assignment</span>}</div>}</div></div></Link>;
