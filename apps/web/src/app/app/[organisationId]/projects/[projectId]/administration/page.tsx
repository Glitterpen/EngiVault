...
export default async function ProjectAdministrationPage({params}:{params:Promise<{organisationId:string;projectId:string}>}){
 const {organisationId,projectId}=await params;const {supabase,actualRole,preview}=await requireProject(organisationId,projectId);if(actualRole!=="organisation_admin"||preview)notFound();
 const [{data:project},{data:policy},{data:backups},{data:connections},{data:members}]=await Promise.all([
