...
type DocumentRow={id:string;document_number:string;title:string;discipline:string;document_type:string;planned_submission_date:string|null;required_issue_status:string|null;responsible_party:string|null};
...
type ProjectRow={code:string;name:string;project_introduction:string|null;key_objectives:string[];planned_start_date:string|null;planned_end_date:string|null};
...
export default async function AssignmentsPage({params,searchParams}:{params:Promise<{organisationId:string;projectId:string}>;searchParams:Promise<{view?:string}>}){
  ...
  const [{data:disciplineRows},{data:documentRows},{data:projectData},{data:noticeData}]=await Promise.all([
    ...
    supabase.from("documents").select("id,document_number,title,discipline,document_type,planned_submission_date,required_issue_status,responsible_party").eq("organisation_id",organisationId).eq("project_id",projectId).eq("lifecycle_status","active").order("discipline").order("document_number").limit(5000),
    supabase.from("projects").select("code,name,project_introduction,key_objectives,planned_start_date,planned_end_date").eq("organisation_id",organisationId).eq("id",projectId).maybeSingle(),
  ...
  const deliverables=documents.map(document=>{
    ...
    return {document,latest,status:engineerDeliverableState({plannedSubmissionDate:document.planned_submission_date,controlStatus:latest?.control_status??null})};
  });
  ...
  const completion=deliverables.length?Math.round(acceptedCount/deliverables.length*100):0;
  ...
  return <div className="mx-auto max-w-[1450px]">
    ...
    <header className="mt-6 flex flex-wrap items-start justify-between gap-5">
      ...
      <div className="min-w-44 rounded-2xl border border-[#dfe7e3] bg-white p-4 shadow-sm"><div className="flex items-end justify-between"><span className="text-3xl font-semibold text-[#0c5b45]">{completion}%</span><FileCheck2 size={22} className="text-[#e8733f]"/></div><p className="mt-1 text-xs font-bold uppercase tracking-[.1em] text-[#617083]">Accepted deliverables</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-[#e5ece8]"><div className="h-full rounded-full bg-[#0c5b45]" style={{width:`${completion}%`}}/></div></div>
    </header>
...
function DeliverableCard({item,organisationId,projectId}:{item:{document:DocumentRow;latest:RevisionRow|null;status:EngineerDeliverableState};organisationId:string;projectId:string}){
  ...
  return <article className={`ev-card overflow-hidden border-l-4 ${config.border}`}><div className="p-5 sm:p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="text-xs font-extrabold text-[#e8733f]">{document.document_number}</p><span className={`rounded-full px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-[.06em] ${config.tone}`}>{config.label}</span></div><h2 className="mt-2 text-lg font-semibold">{document.title}</h2><p className="mt-2 text-xs text-[#617083]">{document.discipline} · {document.document_type}{document.responsible_party?` · ${document.responsible_party}`:""}</p></div><Link href={href} className="ev-button shrink-0"><FileUp size={16}/>{action}</Link></div>
    ...
    <div className="mt-4 grid gap-3 border-t border-[#edf1ef] pt-4 text-xs text-[#617083] sm:grid-cols-3"><span className={`inline-flex items-center gap-1.5 ${status==="overdue"?"font-bold text-[#a5452f]":""}`}><CalendarClock size={14}/> Due {formatDate(document.planned_submission_date)}</span><span className="inline-flex items-center gap-1.5"><ShieldCheck size={14}/> Required: {document.required_issue_status||"To be confirmed"}</span><span className="inline-flex items-center gap-1.5"><FileCheck2 size={14}/>{latest?`Latest ${latest.revision_code} · ${latest.issue_status}`:"No revision submitted"}</span></div>
  </div></article>;
}

const statusConfig:Record<EngineerDeliverableState,{label:string;tone:string;border:string}>={
  accepted:{label:"Accepted",tone:"bg-[#e8f1ed] text-[#0c5b45]",border:"border-l-[#0c5b45]"},
