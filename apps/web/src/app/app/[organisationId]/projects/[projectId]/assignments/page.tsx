...
export default async function AssignmentsPage({params,searchParams}:{params:Promise<{organisationId:string;projectId:string}>;searchParams:Promise<{view?:string}>}){
  ...
  const [{data:disciplineRows},{data:documentRows},{data:projectData},{data:noticeData},{data:impactData}]=await Promise.all([
...
function EngineerProjectImpact({health,impactAvailable,projectActual,projectPlanned,engineerActual,engineerPlanned,engineerShare,contribution,expectedContribution,delayImpact}:{health:EngineerProgressHealth;impactAvailable:boolean;projectActual:number;projectPlanned:number;engineerActual:number;engineerPlanned:number;engineerShare:number;contribution:number;expectedContribution:number;delayImpact:number}){
  ...
  return <section className="mt-6 overflow-hidden rounded-2xl border border-[#dfe7e3] bg-white shadow-sm">
    <div className={`flex flex-wrap items-start justify-between gap-4 border-b px-5 py-4 sm:px-6 ${config.banner}`}>
      <div className="flex items-start gap-3"><span className={`mt-0.5 grid size-10 shrink-0 place-items-center rounded-xl ${config.iconTone}`}>{config.icon}</span><div><p className="text-[10px] font-extrabold uppercase tracking-[.14em]">My project impact</p><h2 className="mt-1 text-lg font-semibold">{config.title}</h2><p className="mt-1 max-w-3xl text-sm leading-6 opacity-80">{health==="no_scope"?"Your discipline is authorised on the project, but the Document Controller has not placed any matching deliverables in the MDR yet.":health==="lagging"?`Your open discipline gap is holding back overall project progress by approximately ${formatPercent(delayImpact)} percentage points. Complete the priority actions below to recover it.`:health==="at_risk"?"Your discipline is currently meeting plan, but deliverables due within seven days could create a project delay if they are not submitted.":"Your discipline is meeting its current planned position and is not creating a negative project schedule impact."}</p></div></div>
...
function CloseOutActions({actions,organisationId,projectId,projectTotalWeight}:{actions:DeliverableItem[];organisationId:string;projectId:string;projectTotalWeight:number}){
  ...
  return <section className="mt-5 rounded-2xl border border-[#dfe7e3] bg-white p-5 shadow-sm sm:p-6">
    <div className="flex flex-wrap items-start justify-between gap-3"><div className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-[#fff0e9] text-[#e8733f]"><ListChecks size={19}/></span><div><p className="text-[10px] font-extrabold uppercase tracking-[.14em] text-[#e8733f]">Gap recovery plan</p><h2 className="mt-1 text-lg font-semibold">Priority close-out actions</h2><p className="mt-1 text-sm text-[#617083]">Work these items from the top down to recover the largest immediate delivery gaps.</p></div></div>{actions.length>0&&<Link href={`?view=action#deliverables`} className="text-xs font-bold text-[#0c5b45] hover:text-[#e8733f]">View all {actions.length} actions</Link>}</div>
    {visible.length?<div className="mt-5 divide-y divide-[#edf1ef] rounded-xl border border-[#e4eae7]">{visible.map((item,index)=>{
      ...
      return <article className="grid gap-3 p-4 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center" key={item.document.id}><span className="grid size-8 place-items-center rounded-full bg-[#10243e] text-xs font-bold text-white">{index+1}</span><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="text-xs font-extrabold text-[#e8733f]">{item.document.document_number}</p><span className={`rounded-full px-2 py-0.5 text-[9px] font-extrabold uppercase ${config.tone}`}>{config.label}</span></div><h3 className="mt-1 truncate font-semibold">{item.document.title}</h3><p className="mt-1 text-xs leading-5 text-[#617083]">{engineerActionInstruction(item.status)}</p><div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[10px] font-bold uppercase tracking-[.06em] text-[#7b8998]"><span>Due {formatDate(due)}</span><span>Potential project recovery {formatPercent(recovery)} pts</span></div></div><Link className="ev-button-secondary justify-self-start sm:justify-self-end" href={`/app/${organisationId}/projects/${projectId}/documents/${item.document.id}#submit-revision`}><FileUp size={15}/>Open action</Link></article>;
