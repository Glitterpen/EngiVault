...
export default async function AppLayout({children}:{children:React.ReactNode}){
  ...
  return <div className="min-h-screen">
    <header className="sticky top-0 z-40 border-b border-[#e2e7ed] bg-white/90 shadow-[0_1px_8px_rgba(16,36,62,.035)] backdrop-blur-xl"><div className="workspace-header-inner mx-auto flex h-16 items-center justify-between px-4 sm:px-6"><Brand href="/app" compact/><div className="flex items-center gap-2 sm:gap-3"><Link href="/app/notifications" className="relative grid size-10 place-items-center rounded-xl border border-[#dde4eb] bg-white text-[#647386] transition hover:border-[#ed7138] hover:bg-[#fff8f4] hover:text-[#ed7138]" title={`${unreadCount??0} unread notifications`} aria-label={`Notifications, ${unreadCount??0} unread`}><Bell size={17}/>{Boolean(unreadCount)&&<span className="absolute -right-1.5 -top-1.5 grid min-w-5 place-items-center rounded-full bg-[#ed7138] px-1 text-[10px] font-extrabold leading-5 text-white ring-2 ring-white">{(unreadCount??0)>99?"99+":unreadCount}</span>}</Link><AccountMenu email={user.email??"Signed-in account"} initialRoleLabel={roleLabel(initialRole)} founderAccess={founderAccess===true}/></div></div></header>
