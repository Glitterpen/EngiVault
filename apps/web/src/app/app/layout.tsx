...
export default async function AppLayout({children}:{children:React.ReactNode}){
  ...
  if(!preview&&roles.includes("executive_viewer")&&!roles.some(role=>["organisation_admin","project_admin","document_controller","engineer"].includes(role))){
    return <div className="min-h-screen"><a href="#workspace-content" className="ev-skip-link">Skip to content</a><header className="border-b border-[#e2e7ed] bg-white"><div className="mx-auto flex min-h-16 max-w-6xl items-center justify-between gap-3 px-4 sm:px-6"><Brand href="/app" compact/><AccountMenu email={user.email??"Signed-in account"} initialRoleLabel="Executive Viewer" founderAccess={founderAccess===true}/></div></header><main id="workspace-content" tabIndex={-1} className="mx-auto max-w-6xl p-4 sm:p-6 lg:p-8">{children}</main></div>;
  }
