...
export async function setDocumentArchived(_:MutationState,form:FormData):Promise<MutationState>{
 ...
 if(error)return {message:!archived&&error.code==="23505"?"This document number is now used by an active MDR deliverable. Edit the removed entry to use a different number before restoring it.":`The MDR deliverable could not be ${archived?"removed":"restored"}. Reference: ${error.code}.`};
