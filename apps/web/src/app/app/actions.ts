...
export async function createOrganisation(_:MutationState,form:FormData):Promise<MutationState>{
 ...
 }else if(error)return {message:serviceFailureMessage("Organisation could not be created. Please try again or contact EngiCite support.",error)};
 ...
 if(uploadError)return {message:serviceFailureMessage("Organisation was created, but its logo could not be stored. Submit the same details again or contact EngiCite support.",uploadError)};
 ...
 if(updateError)return {message:updateError.code==="PGRST202"?"Organisation was created, but its logo could not be linked. Contact EngiCite support before trying again.":`Organisation was created and its logo was stored, but the logo could not be linked. Submit the same details again. Reference: ${supportReference(updateError.code)}.`};
...
export async function createProject(_:MutationState,form:FormData):Promise<MutationState>{
 ...
 if(error){
  ...
  return {message:serviceFailureMessage("Project could not be created. Please try again or contact EngiCite support.",error)};
 }
...
export async function updateOrganisation(_:MutationState,form:FormData):Promise<MutationState>{
 ...
 if(replacementLogo){
  ...
  if(uploadError)return {message:serviceFailureMessage("The replacement logo could not be stored. Please try again.",uploadError)};
 }
 ...
 if(error)return {message:error.code==="23505"?"That URL slug is already used by another organisation.":error.code==="PGRST202"?"Organisation editing is temporarily unavailable. Please contact EngiCite support.":`Organisation could not be updated. Reference: ${supportReference(error.code)}.`};
...
export async function updateProject(_:MutationState,form:FormData):Promise<MutationState>{
 ...
 for(let index=0;index<logos.length;index+=1){
  ...
  if(uploadError)return {message:serviceFailureMessage("A replacement client logo could not be stored. Please try again.",uploadError)};
 }
 ...
 if(error)return {message:error.code==="PGRST202"?"Project editing is temporarily unavailable. Please contact EngiCite support.":`Project could not be updated. Reference: ${supportReference(error.code)}.`};
...
export async function trashProject(_:MutationState,form:FormData):Promise<MutationState>{
 ...
 if(error)return {message:error.code==="22023"?"The project code does not match.":error.code==="42501"?"Only an Organisation Administrator can delete a project.":`Project could not be moved to trash. Reference: ${supportReference(error.code)}.`};
...
export async function updateProjectBackupPolicy(_:MutationState,form:FormData):Promise<MutationState>{
 ...
 if(error)return {message:error.code==="22023"?"The selected external destination is not connected or the schedule is invalid.":`Backup schedule could not be saved. Reference: ${supportReference(error.code)}.`};
...
export async function requestProjectBackup(_:MutationState,form:FormData):Promise<MutationState>{
 ...
 if(error)return {message:error.code==="22023"?"Connect the selected company drive before using it.":`Backup could not be requested. Reference: ${supportReference(error.code)}.`};
 ...
 return {message:built?(parsed.data.provider==="engicite"?"Portable project backup created securely.":"Portable backup created and staged for the selected company drive."):"The backup is queued, but could not be completed yet. Please try again shortly or contact EngiCite support.",mutationId:crypto.randomUUID()}
}
export async function updateDocument(_:MutationState,form:FormData):Promise<MutationState>{const optional=z.string().trim().max(80);const parsed=z.object({organisationId:z.uuid(),projectId:z.uuid(),documentId:z.uuid(),documentNumber:z.string().trim().toUpperCase().min(2).max(80),title:z.string().trim().min(2).max(240),documentType:z.string().trim().min(1).max(80),discipline:z.string().trim().min(1).max(80),area:optional,system:optional,workPackage:optional}).safeParse(Object.fromEntries(form));if(!parsed.success)return {message:"Enter valid document metadata."};const {supabase,access}=await requireProject(parsed.data.organisationId,parsed.data.projectId);if(String(access.role)!=="document_controller")return {message:"Only the appointed Document Controller can modify MDR deliverables."};const {error}=await supabase.rpc("update_document",{target_organisation:parsed.data.organisationId,target_project:parsed.data.projectId,target_document:parsed.data.documentId,new_number:parsed.data.documentNumber,new_title:parsed.data.title,new_type:parsed.data.documentType,new_discipline:parsed.data.discipline,new_area:parsed.data.area,new_system:parsed.data.system,new_work_package:parsed.data.workPackage});if(error)return {message:error.code==="23505"?"That document number already exists in the active MDR for this project.":`Document could not be updated. Reference: ${supportReference(error.code)}.`};const base=`/app/${parsed.data.organisationId}/projects/${parsed.data.projectId}`;revalidatePath(`${base}/documents`);revalidatePath(`${base}/documents/${parsed.data.documentId}`);revalidatePath(`${base}/progress`);return {message:"MDR deliverable updated.",mutationId:crypto.randomUUID()}}
export async function setDocumentArchived(_:MutationState,form:FormData):Promise<MutationState>{
 const parsed=z.object({organisationId:z.uuid(),projectId:z.uuid(),documentId:z.uuid(),archived:z.enum(["true","false"])}).safeParse(Object.fromEntries(form));
 if(!parsed.success)return {message:"The MDR deliverable could not be identified."};
 const {supabase,access}=await requireProject(parsed.data.organisationId,parsed.data.projectId);
 if(String(access.role)!=="document_controller")return {message:"Only the appointed Document Controller can remove or restore MDR deliverables."};
 const archived=parsed.data.archived==="true";
 const {error}=await supabase.rpc("set_document_archived",{target_organisation:parsed.data.organisationId,target_project:parsed.data.projectId,target_document:parsed.data.documentId,archived});
 if(error)return {message:!archived&&error.code==="23505"?"This document number is now used by an active MDR deliverable. Edit the removed entry to use a different number before restoring it.":`The MDR deliverable could not be ${archived?"removed":"restored"}. Reference: ${supportReference(error.code)}.`};
 const base=`/app/${parsed.data.organisationId}/projects/${parsed.data.projectId}/documents`;
 revalidatePath(base);revalidatePath(`${base}/${parsed.data.documentId}`);revalidatePath(`/app/${parsed.data.organisationId}/projects/${parsed.data.projectId}/progress`);
 return {message:archived?"Deliverable removed from the active MDR. Its controlled history remains available in Removed deliverables.":"Deliverable restored to the active MDR.",mutationId:crypto.randomUUID()}
}
export async function createWorkPackage(_:MutationState,form:FormData):Promise<MutationState>{const parsed=z.object({organisationId:z.uuid(),projectId:z.uuid(),packageNumber:z.string().trim().toUpperCase().min(2).max(80),name:z.string().trim().min(2).max(160),purpose:z.string().trim().max(1000),discipline:z.string().trim().max(80),requiredStatus:optionalIssueStatus,destination:z.enum(["local","sharepoint","google_drive"])}).safeParse(Object.fromEntries(form));if(!parsed.success)return {message:"Enter valid package details."};const {supabase}=await requireProject(parsed.data.organisationId,parsed.data.projectId);const {data,error}=await supabase.rpc("create_frozen_work_package",{target_organisation:parsed.data.organisationId,target_project:parsed.data.projectId,new_number:parsed.data.packageNumber,new_name:parsed.data.name,new_purpose:parsed.data.purpose,filter_discipline:parsed.data.discipline,required_status:parsed.data.requiredStatus,target_destination:parsed.data.destination});if(error)return {message:error.code==="23505"?"That package number and version already exist.":`Package could not be created. Reference: ${supportReference(error.code)}.`};redirect(`/app/${parsed.data.organisationId}/projects/${parsed.data.projectId}/work-packages/${data}`)}
export async function createDocumentTransmittal(_:MutationState,form:FormData):Promise<MutationState>{
 ...
 if(error?.code==="55000")return {message:error.message==="transmittal_revision_already_issued"?"One or more revisions are already staged or transmitted. Refresh the list and open the existing transmittal to download or retry it. Only a newly approved revision can enter the next transmission.":"A selected revision has changed or a newer approved revision exists. Refresh the transmission list and select again."};
 if(error)return {message:error.code==="23505"?"That transmittal number already exists.":error.code==="PGRST202"?"Transmittal creation is temporarily unavailable. Please contact EngiCite support.":error.code==="22023"?"The Project Manager must complete the client information before this transmittal can be created.":error.code==="42501"?"One or more selected revisions are no longer accepted and ready. Refresh the page and select again.":`Transmittal could not be created. Reference: ${supportReference(error.code)}.`};
 redirect(`/app/${fields.data.organisationId}/projects/${fields.data.projectId}/work-packages/${data}`)
}
export async function updateDocumentPlan(_:MutationState,form:FormData):Promise<MutationState>{const optionalDate=z.preprocess(v=>v===""?null:v,z.iso.date().nullable());const parsed=z.object({organisationId:z.uuid(),projectId:z.uuid(),documentId:z.uuid(),responsibleParty:z.string().trim().max(80),plannedSubmissionDate:z.iso.date(),plannedFinalDate:optionalDate,requiredIssueStatus:z.string().trim().max(80),progressWeight:z.coerce.number().positive().max(1000)}).safeParse(Object.fromEntries(form));if(!parsed.success)return {message:"Enter the agreed submission date, valid planning details and a positive progress weight."};const {supabase,access}=await requireProject(parsed.data.organisationId,parsed.data.projectId);if(String(access.role)!=="document_controller")return {message:"Only the appointed Document Controller can modify MDR planning details."};if(parsed.data.requiredIssueStatus&&!isDocumentIssueStatus(parsed.data.requiredIssueStatus)){const {data:existing}=await supabase.from("documents").select("required_issue_status").eq("organisation_id",parsed.data.organisationId).eq("project_id",parsed.data.projectId).eq("id",parsed.data.documentId).maybeSingle();if(existing?.required_issue_status!==parsed.data.requiredIssueStatus)return {message:"Choose a controlled issue status from the list."}}const {error}=await supabase.rpc("update_document_plan",{target_organisation:parsed.data.organisationId,target_project:parsed.data.projectId,target_document:parsed.data.documentId,new_responsible:parsed.data.responsibleParty,new_submission:parsed.data.plannedSubmissionDate,new_final:parsed.data.plannedFinalDate,new_required_status:parsed.data.requiredIssueStatus,new_weight:parsed.data.progressWeight});if(error)return {message:`Document plan could not be updated. Reference: ${supportReference(error.code)}.`};const base=`/app/${parsed.data.organisationId}/projects/${parsed.data.projectId}`;revalidatePath(`${base}/documents`);revalidatePath(`${base}/documents/${parsed.data.documentId}`);revalidatePath(`${base}/progress`);return {message:"Document plan updated.",mutationId:crypto.randomUUID()}}
