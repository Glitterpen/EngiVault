...
export default async function AppHome(){
 ...
 if(orgs.length===1&&orgs[0].role==="executive_viewer")redirect(`/app/${orgs[0].organisation_id}/executive`);
 ...
 return <div className="mx-auto max-w-6xl">
  ...
  <div className={`mt-8 grid gap-5 ${canCreateOrganisation?"lg:grid-cols-[1fr_360px]":""}`}>
   <section className="grid content-start gap-3">{orgs.length?orgs.map(org=><article key={org.organisation_id} className="ev-card flex items-center gap-2 p-2 transition hover:-translate-y-0.5">
    <Link href={`/app/${org.organisation_id}${org.role==="executive_viewer"?"/executive":""}`} className="flex min-w-0 flex-1 items-center gap-4 rounded-xl p-3">
     ...
     <div className="min-w-0 flex-1"><h2 className="truncate font-semibold">{org.name}</h2><p className="mt-1 text-xs uppercase tracking-[.1em] text-[#617083]">{org.role==="organisation_admin"?"Organisation Administrator":org.role==="executive_viewer"?"Executive Viewer · Read only":"Project team member"}</p></div><ArrowUpRight size={18}/>
    </Link>
