...
export async function sendInvitationEmail(input:InvitationEmailInput){
  ...
  const from=formatOrganisationSender(configuredFrom,input.organisationName);

  const role=input.role.replaceAll("_"," ");
  const organisationName=sanitiseEmailHeaderText(input.organisationName,"EngiCite");
  const projectName=sanitiseEmailHeaderText(input.projectName,"your EngiCite project");
  ...
  const html=`<div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;color:#10243e"><h1 style="font-size:24px">${heading}</h1><p><strong>${escapeHtml(organisationName)}</strong> has invited you to <strong>${escapeHtml(input.projectName)}</strong> as <strong>${escapeHtml(role)}</strong>${escapeHtml(discipline)}.</p>${brief}${timeline}${accessNote}<p><a href="${escapeHtml(input.acceptUrl)}" style="display:inline-block;background:#e8733f;color:white;text-decoration:none;padding:12px 18px;border-radius:8px;font-weight:bold">Accept invitation</a></p><p style="font-size:12px;color:#617083">This fresh secure link expires in seven days and replaces any earlier link. Sign in using the invited email address. Secure project access is provided through EngiCite.</p></div>`;
