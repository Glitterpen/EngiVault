...
describe("role capabilities",()=>{
  ...
  it("allows engineers to submit only through assignment and discipline-aware upload checks",()=>{
    expect(can("engineer","document:submit_discipline")).toBe(true);
    expect(can("viewer","document:submit_discipline")).toBe(false);
  });
  it("allows document controllers to manage and assign the MDR but not submit revisions",()=>{
    expect(can("document_controller","document:write")).toBe(true);
    expect(can("document_controller","document:assign")).toBe(true);
  ...
  it("allows only document controllers to create MDR entries",()=>{
    ...
    expect(can("document_controller","engineers:manage")).toBe(false);
  ...
  it("separates organisation governance from project execution",()=>{
    ...
    expect(can("project_admin","document:assign")).toBe(false);
  });
  it("limits invitations to the roles each control level appoints",()=>{
    ...
    expect(invitableProjectRoles("document_controller")).toEqual([]);
    ...
    expect(canInviteProjectRole("document_controller","engineer")).toBe(false);
  });
  it("limits appointment removal to the same authority that appoints the role",()=>{
    ...
    expect(canRemoveProjectMember("document_controller","engineer")).toBe(false);
