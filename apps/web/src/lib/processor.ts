...
export async function buildProjectBackup(backupId:string):Promise<boolean>{const base=process.env.PROCESSOR_URL??"http://127.0.0.1:8000";const secret=process.env.PROCESSOR_SHARED_SECRET??"local-development-only";try{const response=await fetch(`${base.replace(/\/$/,"")}/internal/v1/build-project-backup`,{method:"POST",headers:{"content-type":"application/json","x-processor-secret":secret},body:JSON.stringify({backup_id:backupId}),cache:"no-store",signal:AbortSignal.timeout(300000)});return response.ok}catch{return false}}
