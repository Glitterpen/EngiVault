-- DCC-only client transmittals reuse the immutable work-package archive pipeline.
-- Recipient and attestation metadata is frozen into the package manifest so that
-- the generated cover and the selected revision list remain an auditable record.
...
begin
  ...
  if nullif(btrim(new_number), '') is null
     ...
     or nullif(btrim(recipient_company), '') is null
     or char_length(btrim(recipient_company)) > 160
     or nullif(btrim(new_purpose), '') is null
     or char_length(btrim(new_purpose)) > 160
  ...
  select project.code::text, project.name, organisation.name
    into project_code, project_name, organisation_name
  ...
  values(
    ...
    jsonb_build_object(
      ...
      'recipient', jsonb_build_object(
        'company', btrim(recipient_company),
  ...
  values(
    ...
    jsonb_build_object(
      ...
      'recipient_company', btrim(recipient_company),
      'purpose', btrim(new_purpose),
