...
select set_config('request.jwt.claim.sub',pg_temp.person(1)::text,true);
set local role authenticated;
select is((select deletion_state from public.list_removed_organisation_users('82000000-0000-4000-8000-000000000001') where user_id=pg_temp.person(4)),'processing','Processing deletion remains visible until completion');
reset role;
set local role service_role;
...
reset role;
set local role authenticated;
select is((select deletion_state from public.list_removed_organisation_users('82000000-0000-4000-8000-000000000001') where user_id=pg_temp.person(4)),'failed','Failed deletion remains visible for follow-up');
select is((select total_count from public.list_removed_organisation_users('82000000-0000-4000-8000-000000000001') limit 1),7::bigint,'Pending cleanup is included in the account count');
reset role;
set local role service_role;
...
set local role authenticated;
select is((select count(*) from public.list_removed_organisation_users('82000000-0000-4000-8000-000000000001') where user_id=pg_temp.person(4)),0::bigint,'Completed deletion disappears from removed-account list');
select is((select total_count from public.list_removed_organisation_users('82000000-0000-4000-8000-000000000001') limit 1),6::bigint,'Completed deletion is excluded from total count');
select is((select count(*) from public.list_removed_organisation_users('82000000-0000-4000-8000-000000000001','Deleted user')),0::bigint,'Searching cannot bring a completed account back into the list');
select is((select count(*) from public.list_removed_organisation_users('82000000-0000-4000-8000-000000000001','',5)),1::bigint,'Pagination only counts remaining accounts');
select is((select count(*) from public.list_removed_organisation_users('82000000-0000-4000-8000-000000000001','',6)),0::bigint,'Completed account cannot occupy a later page');
reset role;
select is((select count(*) from public.retired_user_accounts where user_id=pg_temp.person(4)),1::bigint,'Hidden completed account retains its anti-reactivation marker');
select is((select storage_key from public.document_revisions where uploaded_by=pg_temp.person(4)),'retained/history.pdf','Hiding the account does not remove submitted files');
